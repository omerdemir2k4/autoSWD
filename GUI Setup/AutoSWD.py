...
class PSDWorker(QObject):
    ...
    finished = pyqtSignal(object)  # {'freqs':..., 'psd1':..., 'psd2':..., 'start_sample': int}
    error = pyqtSignal(str)

    def __init__(self, eeg0, eeg1, sfreq, event, fmin, fmax, bandwidth):
        super().__init__()
        self.eeg0 = eeg0
        self.eeg1 = eeg1
        self.sfreq = sfreq
    ...
    def run(self):
        try:
            ...
            freqs1, psd1 = compute_multitaper_psd(signal_ch1, self.sfreq, self.fmin, self.fmax, self.bandwidth, normalize=False)
            freqs2, psd2 = compute_multitaper_psd(signal_ch2, self.sfreq, self.fmin, self.fmax, self.bandwidth, normalize=False)
...
class PSDAnalysisDialog(QDialog):
    ...
    def init_ui(self):
        ...
        bottom_row.addWidget(QLabel("Export Bins:"))
        ...
        self.export_bins_spin.setFixedWidth(70)
        self.export_bins_spin.setFixedHeight(35)
        bottom_row.addWidget(self.export_bins_spin)
    ...
    def compute_psd(self):
        ...
        self.psd_worker = PSDWorker(self.eeg0, self.eeg1, self.sfreq, self.event, fmin, fmax, bandwidth)
    ...
    def redraw_current_psd_plot(self):
        ...
        psd1 = self.current_psd['ch1'].copy()
        psd2 = self.current_psd['ch2'].copy()

        normalized = self.normalize_cb.isChecked()
        if normalized:
            area1 = np.trapz(psd1, freqs)
            area2 = np.trapz(psd2, freqs)
            if area1 > 0:
                psd1 = psd1 / area1
            if area2 > 0:
                psd2 = psd2 / area2
                ...
                for idx, item in enumerate(self.saved_list.selectedItems()):
                    ...
                    if psd_name in self.saved_psds:
                        ...
                        if len(saved_freqs) != len(freqs) or not np.allclose(saved_freqs, freqs):
                            ...
                            interp_psd1 = saved_psd1.copy()
                        if normalized:
                            area = np.trapz(interp_psd1, freqs)
                            if area > 0:
                                interp_psd1 = interp_psd1 / area
                ...
                for idx, item in enumerate(self.saved_list.selectedItems()):
                    ...
                    if psd_name in self.saved_psds:
                        ...
                        if len(saved_freqs) != len(freqs) or not np.allclose(saved_freqs, freqs):
                            ...
                            interp_psd2 = saved_psd2.copy()
                        if normalized:
                            area = np.trapz(interp_psd2, freqs)
                            if area > 0:
                                interp_psd2 = interp_psd2 / area
                        overlay_color = overlay_colors[idx % len(overlay_colors)]
    ...
    def export_psds(self):
        ...
        try:
            ...
            # Create target frequency array based on union of all saved PSD ranges
...
class AdvancedExportDialog(QDialog):
    ...
    def __init__(self, animals_data, global_settings, recording_type, parent=None,
                 dark_mode=False, existing_settings=None):
        super().__init__(parent)
    ...
    def _setup_ui(self):
        ...
        for animal_name in self.animals_data:
            ...
            if is_injection:
                ...
                edt_inj = QLineEdit(ex.get('injection_time', self.global_settings.get('injection_time', '09:00')) if has_existing else self.global_settings.get('injection_time', '09:00'))
                lbl_end = QLabel("End Time:")
                edt_end = QLineEdit(ex.get('injection_end', self.global_settings.get('injection_end', '12:00')) if has_existing else self.global_settings.get('injection_end', '12:00'))
                ...
                edt_bs = QLineEdit(ex.get('custom_basal_start', '') if has_existing else '')
                edt_bs.setPlaceholderText("e.g. 08:35 (leave empty for default)")
                lbl_be = QLabel("Custom Basal End:")
                edt_be = QLineEdit(ex.get('custom_basal_end', '') if has_existing else '')
                edt_be.setPlaceholderText("e.g. 08:55 (leave empty for default)")

                row = 1
                for lbl, w in [(lbl_inj, edt_inj), (lbl_end, edt_end), (lbl_int, edt_int),
                               (lbl_bs, edt_bs), (lbl_be, edt_be)]:
                ...
                widgets = {
                    ...
                    'all_fields': [lbl_inj, edt_inj, lbl_end, edt_end, lbl_int, edt_int,
                                   lbl_bs, edt_bs, lbl_be, edt_be],
                }
            else:
                lbl_start = QLabel("Start Time:")
                edt_start = QLineEdit(ex.get('basal_start', self.global_settings.get('basal_start', '09:00')) if has_existing else self.global_settings.get('basal_start', '09:00'))
                lbl_end = QLabel("End Time:")
                edt_end = QLineEdit(ex.get('basal_end', self.global_settings.get('basal_end', '12:00')) if has_existing else self.global_settings.get('basal_end', '12:00'))
                lbl_int = QLabel("Interval (min):")
                edt_int = QLineEdit(str(ex.get('interval_minutes', self.global_settings.get('interval_minutes', 20)) if has_existing else self.global_settings.get('interval_minutes', 20)))
                edt_int.setPlaceholderText("e.g. 20")

                row = 1
                for lbl, w in [(lbl_start, edt_start), (lbl_end, edt_end), (lbl_int, edt_int)]:
                    g_lay.addWidget(lbl, row, 0)
                    g_lay.addWidget(w, row, 1)
                    row += 1
                ...
                widgets = {
                    ...
                    'interval_edit': edt_int,
                    'all_fields': [lbl_start, edt_start, lbl_end, edt_end, lbl_int, edt_int],
                }
...
class EnhancedEEGPlotter(QWidget):
    ...
    def on_export_clicked(self):
        ...
        def _open_advanced_dialog():
            ...
            adv_dlg = AdvancedExportDialog(
                ...
                dark_mode=self.dark_mode, existing_settings=self.per_animal_export_settings
            )
            try:
                if self.dark_mode:
    ...
    def _create_details_sheet(self, workbook, animals_info, rec_type, global_settings):
        ...
        model_names = {"cwt_only": "CWT", "unet_only": "UNET", "cwt_unet": "Hybrid (CWT+UNET)"}
    ...
    def _analyze_seizures_injection(self, df, injection_time, interval_minutes=20, basal_duration_min=20, end_time_str='',
        ...
        basal_lbl = "Basal"
        mask = (df[0] >= basal_start) & (df[0] < basal_end_for_mask)
    ...
    def _apply_model_predictions(self):
        ...
        Manual events are always preserved across model switches."""
